...
export default function GradeHomework() {
  ...
  const handleGradeChange = (studentId: string, value: string) => {
    ...
    setGrades(prev => {
      ...
      if (existingGradeIndex >= 0) {
        ...
        const updated = [...prev];
        updated[existingGradeIndex] = { ...updated[existingGradeIndex], grade: numValue };
        return updated;
      } else {
  ...
  const average = filteredStudents.reduce((acc, student) => acc + getStudentGrade(student.id), 0) / filteredStudents.length;
    ...
    >
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">Notes Devoirs</h2>
            <p className="text-slate-500 font-medium">Saisissez et modifiez les notes des devoirs par étudiant</p>
          </div>
                  ...
                  <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <input
                        ...
                        placeholder="Note"
                        className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary font-bold text-slate-800"
                      />
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-3 py-2 text-sm font-medium text-slate-700 bg-slate-100 rounded-lg">
                        {getAppreciation(getStudentGrade(student.id))}
                      </span>
                    </td>
                  </tr>
                ))}
